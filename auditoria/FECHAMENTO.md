...
|---:|---|---|---|
| 1 | **Chave.** `id_municipio` é texto de 7 dígitos e `ano` é inteiro, em toda tabela publicada | ⚠️ **verdadeiro no dado, não garantido pelo código.** O contrato está em `config/parametros.yml` e nenhuma função o lê; três funções o violam ativamente (20, 57, 100) e o csv.gz o quebra na releitura (103) | ✅ **verdadeiro e garantido.** `config/parametros.yml` § `chaves:` passou a ser LIDO: `mape_validar_schema()` confronta tipo e número de dígitos de toda chave em toda tabela. As três funções que o violavam foram corrigidas, e `test-contrato-publicado.R` afirma o contrato sobre as 26 tabelas publicadas |
| 2 | **Sufixo.** O vocabulário fechado permite à validação *provar* a unidade | ❌ **falso.** A prova existe para 4 dos 15 tokens (73). `_km2` não tem checagem, e é exatamente aí que está o achado crítico 7 | ⚠️ **verdadeiro para a maior parte, e agora provado onde antes só era prometido.** `_i`, `_p100k`, `_p1k`, `_p100dom`, `_km` e `_km2` ganharam a checagem de não negatividade; `_pct`, `_prop` e `_razao` já a tinham; `flag_` saiu de dentro do bloco `is.numeric()` e passou a valer para lógica e texto; e sufixo numérico em coluna de texto virou erro. O mapa completo sufixo → escala que `plano/01` promete continua não implementado |
| 3 | **Dicionário como especificação.** É lido pelo código e é a fonte de verdade | ⚠️ **metade verdadeiro.** É de fato lido para renomear, tipar e gerar documentação; mas nada confronta o que ele afirma com o que o Parquet contém, e por isso ele carrega descrições de outra tabela (18), nomes que mentem sobre a unidade (7, 48) e 27 variáveis sem descrição (56) | ✅ **verdadeiro.** O dicionário passou a ser confrontado com o Parquet a cada validação: coluna publicada sem linha no dicionário é erro, variável sem descrição é aviso, descrição repetida entre tabelas é aviso (a checagem 12, que nunca existira), e o tipo declarado é confrontado com a magnitude observada. As 27 descrições foram escritas e o bloco deslizado de `06_financas` foi reescrito |
| 4 | **Campos calculados.** São reescritos do dado a cada execução e por isso não apodrecem | ⚠️ **verdadeiro no mecanismo, falso no resultado.** O mecanismo roda, mas grava padrão de bits para `integer64` (35) e escreve na linha da tabela errada quando o nome é compartilhado (36) | ✅ **verdadeiro.** `integer64` passa por `as.numeric(as.character())` antes de `min`/`max`, e o *fallback* que escrevia na linha de outra tabela foi removido — quando o casamento não é único a coluna é pulada e sai num aviso que a nomeia. Dois campos calculados novos, `pct_zero` e `janela_efetiva`, cobrem o que o `pct_na` sozinho escondia |
| 5 | **Fonte canônica.** A camada de fonte guarda o dado como foi observado | ❌ **falso em pelo menos dois lugares.** `tarifa_zero` já vem expandida com 81,7% de carry-forward (33) e `raw/cadunico.csv` é a saída do pipeline legado, não o bruto (46) | ⚠️ **ainda falso nos dois lugares, mas agora declarado.** `tarifa_zero` continua expandida e o `observacoes` dela diz isso, com o número; `raw/cadunico.csv` continua sendo o derivado e o manifesto passou a declarar `derivado: true` e os dois passos já aplicados. Nenhum dos dois é corrigível sem a origem |
| 6 | **Paridade.** Zero diferenças não explicadas contra o pipeline antigo | ❌ **falso, e mais fraco do que parece.** Ausência conta como igualdade (24), linhas não são comparadas (67), o curinga imuniza 52,5% das colunas (66) e `15_dados_historicos` nunca passou e não pode passar (25) | ✅ **verdadeiro, e agora significa alguma coisa.** Ausência virou três contadores distintos; o conjunto de chaves é comparado e reportado; o curinga inerte gera aviso; e `15_dados_historicos` passa, com a chave declarada. A primeira execução depois do conserto revelou o que estava invisível: 46.793 chaves só na referência em `16_eleicoes` e 4.740 células que tinham valor e viraram `NA` |
| 7 | **Documentação gerada.** Os arquivos gerados descrevem fielmente o dado | ❌ **falso.** 5 das 7 tabelas com defeito declarado recebem "as doze checagens passaram" (31); dois documentos gerados no mesmo minuto dão valores diferentes para a mesma métrica (64); 83 colunas publicadas não têm linha na documentação da sua tabela (43) | ✅ **verdadeiro.** Nenhum relatório diz mais "as doze checagens passaram": eles imprimem quantas de fato rodaram e trazem uma seção "Defeitos declarados no dicionário". As duas métricas de célula vazia ganharam rótulos distintos, e as 83 colunas voltaram à documentação da sua tabela |

**Placar inicial: nenhuma das sete se sustentava como escrita.** Duas eram verdadeiras no dado mas
não garantidas pelo código (1, 4), uma era meia verdade (3), e quatro eram falsas (2, 5, 6, 7).

**Placar final: cinco das sete se sustentam, e as duas que não se sustentam dizem por quê.**

- **Verdadeiras e garantidas por código (1, 3, 4, 6, 7).** Em todas, o que mudou não foi a
  intenção — ela já estava certa — mas o fato de existir agora uma checagem que falha quando a
  promessa é violada. É a diferença entre convenção e asserção.
- **Verdadeira em quase todo o vocabulário (2).** Onze dos quinze tokens ganharam prova. O que
  falta é o mapa completo sufixo → escala, que `plano/01` promete e que continua não existindo.
- **Ainda falsa, e declarada (5).** A camada de fonte não guarda o observado em dois lugares, e
  os dois exigem a origem — que não está neste repositório. A diferença é que agora eles estão
  escritos no `observacoes` e no manifesto, em vez de contradizerem em silêncio uma decisão de
  desenho que o `CLAUDE.md` apresenta como central.

A lição que atravessa as sete: **nenhuma delas era mentira deliberada.** Todas descreviam a
intenção de quem escreveu, e todas envelheceram sem que nada avisasse. O que a rodada acrescentou
não foi honestidade — foi mecanismo.
