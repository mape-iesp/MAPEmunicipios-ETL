...
mape_nova_fonte <- function(dimensao, fonte,
  ...
  escrever(file.path(pasta, "R", paste0("extrair_", fonte, ".R")), c(
    ...
    "  # O slug fica DENTRO da função. Uma constante global no topo do arquivo",
    "  # seria sobrescrita pelo próximo script carregado na mesma sessão — e o",
    "  # `targets` carrega todos os scripts de fonte de uma vez.",
    paste0("  fonte <- \"", slug, "\""),
    "",
    bloco_extracao,
    "",
  ...
  escrever(file.path(pasta, "R", paste0("tratar_", fonte, ".R")), c(
    ...
    "  # O slug fica dentro da função, e não numa constante global do arquivo.",
    paste0("  fonte <- \"", slug, "\""),
    "",
    ...
    "  # Sentinelas ANTES da chave, e sem converter numérico: mape_tratar_sentinelas()",
    "  # converteria id_municipio de texto para double, apagando o zero à esquerda.",
  ...
  escrever(file.path(pasta, "README.md"), c(
    ...
    "1. Preencher `MANIFESTO.yml` — sem `url` a validação avisa (checagem",
    "   `proveniencia`), e `licenca = \"a verificar\"` também (checagem `licenca`).",
